...
public class Operator {
    ...
    public static final Operator OP_ADDITION;
    public static final Operator OP_SUBTRACTION;
    public static final Operator OP_MULTIPLICATION;
    public static final Operator OP_DIVISION;
    public static final Operator OP_POWER;
    public static final Operator OP_MODULO;
    public static final Operator OP_UNARY_MINUS;
    public static final Operator OP_UNARY_PLUS;
    ...
    static {
        OP_ADDITION = new Operator("+", 2, true, PRECEDENCE_ADDITION, args -> args[0] + args[1]);
        OP_SUBTRACTION = new Operator("-", 2, true, PRECEDENCE_ADDITION, args -> args[0] - args[1]);
        OP_UNARY_MINUS = new Operator("-", 1, false, PRECEDENCE_UNARY_MINUS, args -> -args[0]);
        OP_UNARY_PLUS = new Operator("+", 1, false, PRECEDENCE_UNARY_PLUS, args -> args[0]);
        OP_MULTIPLICATION = new Operator("*", 2, true, PRECEDENCE_MULTIPLICATION, args -> args[0] * args[1]);
        OP_DIVISION = new Operator("/", 2, true, PRECEDENCE_DIVISION, args -> {
            if (args[1] == 0d) {
                throw new ArithmeticException("Division by zero!");
            }
            return args[0] / args[1];
        });
        OP_POWER = new Operator("^", 2, false, PRECEDENCE_POWER, args -> Math.pow(args[0], args[1]));
        OP_MODULO = new Operator("%", 2, true, PRECEDENCE_MODULO, args -> {
            if (args[1] == 0d) {
                throw new ArithmeticException("Division by zero!");
            }
            return args[0] % args[1];
    ...
    public static Operator get(final char symbol, final int numArguments) {
        switch (symbol) {
            case '+': return numArguments == 1 ? OP_UNARY_PLUS : OP_ADDITION;
            case '-': return numArguments == 1 ? OP_UNARY_MINUS : OP_SUBTRACTION;

            case '\u00F7': case '/': return OP_DIVISION;
     ...
     * @param exp operation on the given operands
     */
    public Operator(String symbol, int numberOfOperands, boolean leftAssociative, int precedence, IExpression exp) {
